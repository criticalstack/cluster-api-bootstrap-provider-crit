...
func TestMatchesConfigurationHash(t *testing.T) {
	t.Run("machine with configuration hash returns true", func(t *testing.T) {
		g := NewWithT(t)
		m := &clusterv1.Machine{}
		m.SetLabels(internal.ControlPlaneLabelsForClusterWithHash("test", "test", "hashValue"))
	...
	t.Run("machine with wrong configuration hash returns false", func(t *testing.T) {
		g := NewWithT(t)
		m := &clusterv1.Machine{}
		m.SetLabels(internal.ControlPlaneLabelsForClusterWithHash("test", "test", "notHashValue"))
