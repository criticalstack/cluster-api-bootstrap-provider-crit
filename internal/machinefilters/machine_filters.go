...
func OwnedControlPlaneMachines(controlPlaneName string) Func {
	return func(machine *clusterv1.Machine) bool {
		...
		return machine.Labels[clusterv1.MachineControlPlaneLabelName] == controlPlaneName
	}
}
...
// that match a given CritControlPlane configuration hash.
func MatchesConfigurationHash(configHash string) Func {
	return func(machine *clusterv1.Machine) bool {
		if machine == nil {
			return false
		}
		if hash, ok := machine.Labels[controlplanev1.CritControlPlaneHashLabelKey]; ok {
